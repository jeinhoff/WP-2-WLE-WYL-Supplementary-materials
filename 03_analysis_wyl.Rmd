...
# Employed, Retired, Unemployed, Ill health, Other
...
table_1 <- table_1 %>% mutate(outcome = ifelse(status == "Other", 1, 0))
...
ggplot(table_1, aes(x = as.factor(cohort_two), y = WLE, group = country_code, country = country_code)) +
  ...
  scale_y_continuous("WYL in years (from age 55 to 64)", breaks = seq(-10, 10, 1)) +
...
ggsave("outputs/wyl_4_inactivity.png", width = 9, height = 6)
