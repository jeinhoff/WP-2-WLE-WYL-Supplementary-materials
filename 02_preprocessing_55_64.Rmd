...
load("outputs/data_raw_55_64.RData")
...
table <- table %>% group_by(country, cohort, gender) %>% filter(n() == 10 | cohort > 1955)
...
```{r}
data <- data %>% filter(!country_cohort %in% c("DK_1928", "DK_1929") & !country %in% c("LU"))
...
```{r}
data <- data %>% 
  select(country, country_code, country_group, gender, age, year, cohort, cohort_two, cohort_two_nb, education, status, working_hours, working_hours_usual, survival, weight) %>%
